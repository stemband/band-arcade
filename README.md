...
- **DIFFICULTY: NORMAL | EXTRA SPOOKY** (remembered on the level screen). EXTRA SPOOKY is the same eight showtimes with bigger counts and a band that walks 15–20% faster: The 5:00 Show × 2–3, 6:00 × 4, 7:00 × 5, 8:00 × 6, 9:00 × 6–8, 10:00 × 8, 11:00 × 8–10, and The Midnight Encore with Maestro Moose at × 12, three times over. Snare Drum counts go up the same way, to × 16. Same number of animatronics and lanes. It's **locked** ("Clear The 5:00 Show to unlock") until that instrument clears The 5:00 Show on Normal, in any mode (always open in `?demo`); the first time it opens, the results screen says so. Both columns are in `showtime-malfunction/levels.js` (each showtime's `x`: `count`, `snare`, `speed`, `blurb`, and the boss's `count`/`snare`), so you can edit Normal and EXTRA SPOOKY side by side. **Stars are kept separately**: EXTRA SPOOKY saves under the same progress keys plus `:extra` (`showtime-malfunction:extra`, `showtime-malfunction:scale-Eb:extra`, `showtime-malfunction:count:extra`…), and the level screen shows the stars for the difficulty you picked. Both count toward the instrument's star total (the arcade floor, the player card, star skins); the game's maximum is 48 (24 + 24).
- **SPOOKY LEVEL** (remembered on the level screen, separate from the difficulty: it only changes how things look, never the counts): **Mild** (default) = glitches and static only; **Spooky** = darker, more static, and a sudden silent lean-in at SHOWTIME'S OVER. No screams, and the flicker is always slow and gentle; with reduced motion there's no flicker, twitching or lean-in at all. The first switch to EXTRA SPOOKY sets Spooky (you can switch back to Mild); EXTRA SPOOKY with Spooky looks a little darker still: red emergency lighting that slowly pulses, more static on the cabinets and eyes that flicker more.
...
- **Achievements:** defeat Maestro Moose to unlock the **Animatronic** skin for every instrument; defeat him on EXTRA SPOOKY for the **Nightmare Animatronic** skin.
...
- **Select Player:** character select music (`select-music`, a loop; until you upload one, a built-in original chiptune plays; a game can have its own, `select-music-<game>`, e.g. `select-music-sustain-speedway`), `tile-move`, `player-select`, `player-continue`, `player-ready`, `player2-join`, and the skin sounds. The music replaces the room ambience there and fades out as the game opens.
...
| Nightmare Animatronic | clear The Midnight Encore on EXTRA SPOOKY in Showtime Malfunction: cracked chrome, bolts, a few loose wires, and one red eye and one blue that swap back and forth |
